...
public class MecanimEventsSMB : StateMachineBehaviour
{
    ...
    private void TriggerEvent (Animator animator, MecanimEvent mecanimEvent)
    {
        ...
        MecanimEvent.TriggerEvent (GetParent(animator.transform, Upwards).gameObject, mecanimEvent);
    }

    private static Transform GetParent(Transform transform, int depth)
    {
        while (depth > 0)
        {
            depth--;
